 ...
 * @returns {Object} Generated JSON configuration
 */
export const generateRuleEngineJson = (nodes, edges) => {
    try {
        // Find the initial node
        const initialNode = nodes.find(node => node.type === 'initial');
        if (!initialNode) {
            throw new Error('No Initial Node found. Please add an Initial Node first.');
        }
        ...
        if (ruleGroups.length === 0) {
            throw new Error('No Rule Groups found. Please add and connect Rule Groups to the Initial Node.');
        }

        // Generate rules array
        const rules = ruleGroups.map(ruleGroup => {
            return generateRuleFromGroup(ruleGroup, nodes, edges);
        }).filter(rule => rule !== null);

        if (rules.length === 0) {
            throw new Error('No valid rules found. Please ensure all Rule Groups have proper connections and data.');
        }
 ...
 * Find all rule groups connected to the initial node
 */
const findConnectedRuleGroups = (initialNodeId, nodes, edges) => {
    const connectedRuleGroups = [];
    ...
    for (const edge of initialEdges) {
        ...
        if (targetNode && targetNode.type === 'ruleName') {
            ...
            const ruleGroup = nodes.find(node =>
 ...
 */
const generateRuleFromGroup = (ruleGroup, nodes, edges) => {
    try {
        // Find the rule name node within this group
        const ruleNameNode = nodes.find(node =>
            node.type === 'ruleName' &&
        ...
        const ruleName = ruleNameNode.data?.ruleName || ruleNameNode.data?.value || 'UnnamedRule';
        ...
        const conditionalOperatorNodes = findConditionalOperatorsForGroup(ruleGroup.id, nodes, edges);

        // Generate expression from conditions and operators
        const expression = generateExpression(conditionNodes, conditionalOperatorNodes);
 ...
 */
const findConditionalOperatorsForGroup = (ruleGroupId, nodes, edges) => {
    ...
    const childConditionalOperators = nodes.filter(node =>
 ...
 */
const generateExpression = (conditionNodes, conditionalOperatorNodes) => {
    if (conditionNodes.length === 0) {
        return '';
    }
 ...
 */
const generateConditionExpression = (conditionNode) => {
    ...
    if (!table || !field || !expression || !value) {
        console.warn(`Incomplete condition data for node ${conditionNode.id}:`, data);
        return '';
    }
 ...
 */
const getFieldType = (table, field) => {
 ...
 */
const getExpressionSymbol = (expression, fieldType) => {
 ...
 */
const getOperatorForConditions = (conditionalOperatorNodes) => {
    // Find the operator from conditional operator nodes
    if (conditionalOperatorNodes.length > 0) {
        const operator = conditionalOperatorNodes[0].data?.operator || conditionalOperatorNodes[0].data?.selectedOperator || 'AND';
 ...
 */
const findConnectedActionGroup = (ruleGroupId, nodes, edges) => {
    // First, try to find action groups that are children of the rule group
    const childActionGroups = nodes.filter(node =>
        node.type === 'resizableGroup' &&
        node.parentId === ruleGroupId &&
    ...
    for (const edge of ruleEdges) {
        ...
        if (targetNode && targetNode.type === 'conditionalOperator') {
            ...
            for (const operatorEdge of operatorEdges) {
                const actionTargetNode = nodes.find(node => node.id === operatorEdge.target);
                if (actionTargetNode && actionTargetNode.type === 'actionName') {
                    // Find the parent action group
                    const actionGroup = nodes.find(node =>
                        node.type === 'resizableGroup' &&
 ...
 */
const generateActionsFromGroup = (actionGroup, nodes) => {
    // Find all action name nodes within this action group
    const actionNameNodes = nodes.filter(node =>
        node.type === 'actionName' &&
    ...
    actionNameNodes.forEach(actionNameNode => {
        const data = actionNameNode.data || {};
        const actionType = data.actionType || data.selectedActionType || 'OnSuccess';
        const actionName = data.actionName || data.inputActionName || 'DefaultAction';

        // Find condition nodes connected to this action
        const conditionNodes = findConditionNodesForAction(actionNameNode.id, nodes);

        // Generate expression from condition nodes
        const conditionExpression = generateActionConditionExpression(conditionNodes);
 ...
 */
const findConditionNodesForAction = (actionNodeId, nodes) => {
    // Find all condition nodes within the same action group
    const actionGroupId = nodes.find(node => node.id === actionNodeId)?.parentId;

    return nodes.filter(node =>
        node.type === 'condition' &&
        node.parentId === actionGroupId
    );
};
 ...
 */
const generateActionConditionExpression = (conditionNodes) => {
    if (conditionNodes.length === 0) {
        return '';
    }

    const expressionParts = conditionNodes.map(conditionNode => {
